...
export function Landing() {
  return (
    <div className="flex flex-col min-h-screen bg-white">
     ...
     <section className="relative bg-slate-950 text-white overflow-hidden min-h-[600px] sm:min-h-[650px] md:min-h-[700px] lg:min-h-[800px] flex items-center">
  {/* Fond avec motif */}
  ...
  {/* Bouton secondaire - Devenir conducteur */}
  <Link href="/sign-up?role=chauffeur" className="w-full sm:w-1/2">
    ...
    >
      ...
      {/* Colonne droite - Illustration de la carte */}
      ...
      <section className="py-16 sm:py-20 lg:py-24 px-4 sm:px-6 lg:px-8 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16 lg:mb-20">
            <h2 className="text-3xl sm:text-4xl font-bold text-slate-900 mb-4 sm:mb-6">Pourquoi choisir CamionGo ?</h2>
            <p className="text-lg sm:text-xl text-slate-600 px-4 sm:px-0">Une plateforme gagnant-gagnant conçue pour optimiser la logistique des entreprises et maximiser les revenus des transporteurs.</p>
          </div>

          <div className="grid lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12">
            {/* Pour les PME */}
            <div className="bg-slate-50 p-6 sm:p-8 lg:p-10 rounded-3xl border border-slate-100 flex flex-col">
              <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8 lg:mb-10 border-b border-slate-200 pb-4 sm:pb-6">
                ...
                <h3 className="text-2xl sm:text-3xl font-bold text-slate-900">Pour les PME & Expéditeurs</h3>
              </div>
                  ...
                  <div key={idx} className="flex gap-4 sm:gap-5">
                    <div className="shrink-0 mt-1">
                      <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-blue-100 flex items-center justify-center">
                        <item.icon className="w-5 h-5 sm:w-6 sm:h-6 text-blue-700" />
                      </div>
                    </div>
                    <div>
              ...
              <div className="mt-8 sm:mt-10 lg:mt-12 pt-6 sm:pt-8 border-t border-slate-200">
                <Link href="/sign-up?role=entreprise" className="inline-block w-full sm:w-auto">
                  <Button className="w-full gap-2 bg-slate-900 text-white hover:bg-slate-800 h-10 sm:h-12 px-6 sm:px-8 text-sm sm:text-base rounded-xl">
            ...
            {/* Pour les Chauffeurs */}
            <div className="bg-slate-900 p-6 sm:p-8 lg:p-10 rounded-3xl border border-slate-800 text-white flex flex-col">
      ...
      <section className="py-16 sm:py-20 lg:py-24 px-4 sm:px-6 lg:px-8 bg-slate-50">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-3xl sm:text-4xl font-bold text-center text-slate-900 mb-10 sm:mb-12 lg:mb-16">Comment ça marche ?</h2>
          
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-8 relative">
            {/* Connecting line - caché sur mobile */}
            <div className="hidden lg:block absolute top-12 left-[15%] right-[15%] h-0.5 bg-blue-200 z-0"></div>
            ...
            {[
              ...
              <div key={idx} className="relative z-10 flex flex-col items-center text-center px-4">
                <div className={`w-20 h-20 sm:w-24 sm:h-24 rounded-full border-4 border-blue-100 flex items-center justify-center mb-4 sm:mb-6 shadow-sm ${idx === 2 ? 'bg-blue-700' : 'bg-white'}`}>
                  <item.icon className={`w-8 h-8 sm:w-10 sm:h-10 ${idx === 2 ? 'text-white' : 'text-blue-700'}`} />
                </div>
                <h3 className="text-xl sm:text-2xl font-bold text-slate-900 mb-2 sm:mb-3">{item.title}</h3>
                <p className="text-base sm:text-lg text-slate-600">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 sm:py-20 lg:py-24 px-4 sm:px-6 lg:px-8 bg-white">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-3xl sm:text-4xl font-bold text-slate-900 mb-2 sm:mb-4">Types de véhicules disponibles</h2>
          <p className="text-lg sm:text-xl text-slate-600 mb-8 sm:mb-10 lg:mb-12 max-w-2xl">Une flotte diversifiée pour répondre à tous vos besoins logistiques, du petit colis au chargement exceptionnel.</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {[
              ...
              <div key={idx} className="bg-slate-50 p-5 sm:p-6 rounded-xl border border-slate-100 hover:border-blue-200 transition-colors">
                <vehicle.icon className="w-8 h-8 sm:w-10 sm:h-10 text-blue-700 mb-3 sm:mb-4" />
                <h3 className="text-lg sm:text-xl font-bold text-slate-900 mb-1 sm:mb-2">{vehicle.title}</h3>
                <p className="text-sm sm:text-base text-slate-600">{vehicle.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 text-left">
            {[
              ...
              <div key={idx} className="bg-slate-800 p-5 sm:p-6 lg:p-8 rounded-2xl">
      ...
      <section className="py-16 sm:py-20 lg:py-24 px-4 sm:px-6 lg:px-8 bg-slate-50">
        <div className="max-w-7xl mx-auto">
          <div className="text-center max-w-3xl mx-auto mb-10 sm:mb-12 lg:mb-16">
            <h2 className="text-3xl sm:text-4xl font-bold text-slate-900 mb-4 sm:mb-6">Ils font confiance à CamionGo</h2>
            <p className="text-lg sm:text-xl text-slate-600 px-4">Découvrez comment notre plateforme transforme le quotidien des entreprises et des transporteurs à travers tout le Bénin.</p>
          </div>
          
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 mb-12 sm:mb-16">
            {/* Témoignage 1 */}
            <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow flex flex-col">
              <div className="flex gap-1 mb-4 sm:mb-6">
                {[...Array(5)].map((_, i) => <Star key={i} className="w-4 h-4 sm:w-5 sm:h-5 fill-blue-600 text-blue-600" />)}
              </div>
              <p className="text-base sm:text-lg text-slate-700 italic mb-6 sm:mb-8 flex-grow">
              ...
              <div className="flex items-center gap-3 sm:gap-4 mt-auto">
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-blue-100 rounded-full flex items-center justify-center font-bold text-blue-700 shrink-0">BM</div>
                <div>
                  <p className="font-bold text-slate-900 text-sm sm:text-base">BTP Matériaux</p>
                  <p className="text-xs sm:text-sm text-slate-500">Expéditeur régulier</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow flex flex-col">
              ...
              <div className="flex items-center gap-3 sm:gap-4 mt-auto">
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-blue-100 rounded-full flex items-center justify-center font-bold text-blue-700 shrink-0">JP</div>
                <div>
                  <p className="font-bold text-slate-900 text-sm sm:text-base">Jean-Pierre D.</p>
                  <p className="text-xs sm:text-sm text-slate-500">Chauffeur indépendant</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow flex flex-col">
              ...
              <div className="flex items-center gap-3 sm:gap-4 mt-auto">
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-blue-100 rounded-full flex items-center justify-center font-bold text-blue-700 shrink-0">CA</div>
                <div>
                  <p className="font-bold text-slate-900 text-sm sm:text-base">Coopérative Agricole</p>
                  <p className="text-xs sm:text-sm text-slate-500">Producteur local</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-50 rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-10 flex flex-col md:flex-row items-center justify-between border border-blue-100 gap-4 sm:gap-6">
            <div className="mb-4 md:mb-0 md:mr-8 text-center md:text-left">
              <h3 className="text-xl sm:text-2xl font-bold text-slate-900 mb-2">Vous utilisez déjà CamionGo ?</h3>
              <p className="text-base sm:text-lg text-slate-600">Partagez votre expérience avec la communauté et aidez-nous à améliorer nos services.</p>
            </div>
            <Button size="lg" variant="outline" className="shrink-0 bg-white border-2 border-blue-200 text-blue-700 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-colors font-semibold text-sm sm:text-base px-6 sm:px-8">
              Laisser un avis
            </Button>
          </div>
        </div>
      ...
      {/* CTA Final */}
      <section className="py-16 sm:py-20 lg:py-24 px-4 sm:px-6 lg:px-8 bg-blue-700 text-white text-center">
